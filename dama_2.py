...
def place_stones(ws,bs):
    ...
    w_used_positions=[]
    b_used_positions=[]                 #pro kontrolu použitých polí
    ...
    with open(filename,"r") as csvfile:
        ...
        for i in range(1,25):                       
            #kontrola kamenů            
            if ((csvreader[i][0] in available_positions) & (csvreader[i][1]=="w")):       #v pořádku, políčko ano, barva ano
                ws[w_count].center(center_positions[csvreader[i][0]])
                w_count=w_count+1
            elif ((csvreader[i][0] in available_positions) & (csvreader[i][1]=="b")):         #v pořádku, políčko ano, barva ano                
...
def create_board(width, height,ws,bs):
    ...
    screen = pg.display.set_mode((width, height))        #velikost okna
    clock = pg.time.Clock()
    ...
    for y in range(0, height, tile_size):        
        for x in range(0, width, tile_size):            
            ...
            #vykreslení kamenů
            for i in range(12):
                pg.draw.circle(background, 'white', ws[i].get_center(),40)                

            for i in range(12):
                pg.draw.circle(background, 'gray', bs[i].get_center(),40)                
    ...
    while not game_exit:
        ...
        clock.tick(30)                          #zjistit, nemusí tu být        
...
def main():
    ...
    if place_stones(w_stones,b_stones)==True:
        create_board(840,840,w_stones,b_stones)
        print_stones(w_stones,b_stones)
